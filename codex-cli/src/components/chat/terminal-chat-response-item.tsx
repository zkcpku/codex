...
export function Markdown({
  ...
  const rendered = React.useMemo(() => {
    ...
    const marked = new Marked(
      markedTerminal(rendererOptions) as never,
    );
    const parsed = postprocessInlineMarkdown(
      (marked.parse(linkifiedMarkdown) as string).trim(),
    );
...
function postprocessInlineMarkdown(rendered: string): string {
  return rendered
    .split("\n")
    .map((line) =>
      line
        .replace(/(?<!\u001b)\[([^\]]+)\]\(([^)]+)\)/g, (_match, label, href) => {
          return chalk.blue(`${label} (${chalk.underline(href)})`);
        })
        .replace(/`([^`]+)`/g, (_match, text) => chalk.yellow(text))
